...
# History Log:
# 04/28/2023: Initial script assembled
###############################################################################
...
if [ ${MODELNAME} = rrfs ]; then
   ...
   export CONUS_INPUT_TEMPLATE=${modsys}.{init?fmt=%Y%m%d}/{init?fmt=%H}/${modsys}.t{init?fmt=%2H}z.prslev.3km.f{lead?fmt=%3H}.conus.grib2
...
while [ $fhr -le $fhr_max ]; do
   ...
   if [ ${MODELNAME} = rrfs ]; then
      export conus_file=${modsys}.${IDATE}/${INIT_HR}/${modsys}.t${INIT_HR}z.prslev.3km.f$(printf "%03d" $fhr).conus.grib2
   fi

...
model1=`echo nam_firewxnest | tr a-z A-Z`
model2=`echo rrfs_firewxnest | tr a-z A-Z`
export model1
export model2
...
while [ $DATE -ge $ENDDATE ]; do
	...
	if [ -e ${EVSINnam}.$DAY/evs.stats.nam_firewxnest.${RUN}.${VERIF_CASE}.v${DAY}.stat ]
	then
	 cpreq ${EVSINnam}.$DAY/evs.stats.nam_firewxnest.${RUN}.${VERIF_CASE}.v${DAY}.stat $STATDIR

	 sed "s/$model1/nam_firewxnest/g" $STATDIR/evs.stats.nam_firewxnest.${RUN}.${VERIF_CASE}.v${DAY}.stat > $STATDIR/temp.stat
	 ...
	 mv $STATDIR/temp4.stat $STATDIR/evs.stats.nam_firewxnest.${RUN}.${VERIF_CASE}.v${DAY}.stat
	 rm -f $STATDIR/temp*stat
	fi
	
    if [ -e ${EVSINrrfs}.$DAY/evs.stats.rrfs_firewxnest.${RUN}.${VERIF_CASE}.v${DAY}.stat ]
	then
	 cpreq ${EVSINrrfs}.$DAY/evs.stats.rrfs_firewxnest.${RUN}.${VERIF_CASE}.v${DAY}.stat $STATDIR

	 sed "s/$model2/rrfs_firewxnest/g" $STATDIR/evs.stats.rrfs_firewxnest.${RUN}.${VERIF_CASE}.v${DAY}.stat > $STATDIR/temp.stat
	 sed "s/FULL/FireWx/g" $STATDIR/temp.stat > $STATDIR/temp2.stat
	 sed "s/TDO/DPT/g" $STATDIR/temp2.stat > $STATDIR/temp3.stat
	 sed "s/MXGS/GUST/g" $STATDIR/temp3.stat > $STATDIR/temp4.stat
	 mv $STATDIR/temp4.stat $STATDIR/evs.stats.rrfs_firewxnest.${RUN}.${VERIF_CASE}.v${DAY}.stat
	 rm -f $STATDIR/temp*stat
	fi
	...
	if [ -e /lfs/h2/emc/vpppg/noscrub/marcel.caron/$NET/$evs_ver_2d/stats/$COMPONENT/rrfs.$DAY/evs.stats.rrfs.${RUN}.${VERIF_CASE}.v${DAY}.stat ]; then
	 cpreq /lfs/h2/emc/vpppg/noscrub/marcel.caron/$NET/$evs_ver_2d/stats/$COMPONENT/rrfs.$DAY/evs.stats.rrfs.${RUN}.${VERIF_CASE}.v${DAY}.stat $STATDIR
	fi

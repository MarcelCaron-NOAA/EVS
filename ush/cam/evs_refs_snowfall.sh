    ...
    for fhr in $fhrs; do
	   ...
	   if [ -s $input_fcst ] && [ -s $input_obsv ] ; then
	    ...
	    echo "set -x" >> run_refs_snow${obsv}.${fhr}.${vhr}.sh  
            ...
            echo  "export output_base=$WORK/precip/run_refs_snow${obsv}.${fhr}.${vhr}" >> run_refs_snow${obsv}.${fhr}.${vhr}.sh
            ...
            echo  "  export err=\$?; err_chk" >> run_refs_snow${obsv}.${fhr}.${vhr}.sh
	    ...
	    #Mark job is completed
            echo "for FILEn in \$output_base/stat/\${MODEL}/*_stat_\${MODEL}_${obsv}_*_${fhr}0000L_${VDATE}_${vhr}0000V.stat; do if [ -f \"\$FILEn\" ]; then cp -v \$FILEn $all_stats/REFS_SNOW; fi; done" >> run_refs_snow${obsv}.${fhr}.${vhr}.sh
            echo " [[ \$? = 0 ]] && echo completed >\$output_base/stat/REFS_SNOW/run_refs_snow${obsv}.${fhr}.${vhr}.completed" >> run_refs_snow${obsv}.${fhr}.${vhr}.sh

            #Send restart files to COMOUT 
	    echo "if [ $SENDCOM = YES ] && [ \$? = 0 ] ; then" >> run_refs_snow${obsv}.${fhr}.${vhr}.sh
	    echo " if [ -s $all_stats/REFS_SNOW/*_stat_\${MODEL}_${obsv}_*_${fhr}0000L_${VDATE}_${vhr}0000V.stat ] ; then"  >> run_refs_snow${obsv}.${fhr}.${vhr}.sh
	    echo "   cp $all_stats/REFS_SNOW/*_stat_\${MODEL}_${obsv}_*_${fhr}0000L_${VDATE}_${vhr}0000V.stat $COMOUTsmall/REFS_SNOW" >> run_refs_snow${obsv}.${fhr}.${vhr}.sh
	    echo "   cp \$output_base/stat/REFS_SNOW/run_refs_snow${obsv}.${fhr}.${vhr}.completed $COMOUTrestart/snow" >> run_refs_snow${obsv}.${fhr}.${vhr}.sh
            echo " fi" >> run_refs_snow${obsv}.${fhr}.${vhr}.sh
            echo "fi" >> run_refs_snow${obsv}.${fhr}.${vhr}.sh

	    chmod +x run_refs_snow${obsv}.${fhr}.${vhr}.sh
            echo "${DATA}/scripts/run_refs_snow${obsv}.${fhr}.${vhr}.sh" >> run_all_refs_snowfall_poe.sh
         
	   fi   
	  else
	    if [ -s $COMOUTsmall/REFS_SNOW/*_stat_REFS_SNOW_${obsv}_*_${fhr}0000L_${VDATE}_${vhr}0000V.stat ] ; then
	      [[ ! -d $all_stats/REFS_SNOW ]] && mkdir -p $all_stats/REFS_SNOW
	      cp $COMOUTsmall/REFS_SNOW/*_stat_REFS_SNOW_${obsv}_*_${fhr}0000L_${VDATE}_${vhr}0000V.stat $all_stats/REFS_SNOW
            fi 	      

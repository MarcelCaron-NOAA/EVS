...
if [ ${MODELNAME} = hireswarw ]; then
   ...
   if [[ "$mem" == "ctl" ]]; then
      export MODEL_INPUT_TEMPLATE=arafs.{init?fmt=%Y%m%d}/{init?fmt=%H}/${modsys}.t{init?fmt=%2H}z.prslev.3km.f{lead?fmt=%3H}.${DOM}.grib2
   else
      export MODEL_INPUT_TEMPLATE=arefs.{init?fmt=%Y%m%d}/{init?fmt=%H}/m00${mem}/${modsys}.t{init?fmt=%2H}z.m00${mem}.prslev.3km.f{lead?fmt=%3H}.${DOM}.grib2
   fi
...
while [ $fhr -le $fhr_max ]; do
   ...
   if [ ${MODELNAME} = hireswarw ]; then
      ...
      if [[ "$mem" == "ctl" ]]; then
          ...
          export fcst_file=arefs.${IDATE}/${INIT_HR}/m00${mem}/${modsys}.t${INIT_HR}z.m00${mem}.prslev.3km.f$(printf "%03d" $fhr).${DOM}.grib2
      fi
   fi
...
export obs_file=${RUN}.${VDATE}/mrms/${DOMAIN}/${MRMS_PRODUCT}_${OBS_PROD}_${VDATE}-${vhr}0000.${VERIF_GRID}.nc
